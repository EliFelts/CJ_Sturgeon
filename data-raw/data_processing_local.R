...
deploy_locations.df <- read_excel("data-raw/deployment_locations.xlsx") |>
...
deployments.df <- read_excel("data-raw/deployment_data.xlsx") |>
...
fish.df <- read_excel(path = "data-raw/Telemetry_TaggedSturgeon_Info.xlsx") |>
  mutate(
...
#
write_feather(
  hourly_fish_interpolated,
  "data-raw/hourly_fish"
)
...
valid_temp_days <- filtered_fish_detections |>
  filter(
    !is.na(real_sensor),
...
valid_temp_detections <- filtered_fish_detections |>
  semi_join(valid_temp_days, by = c("fish_id", "detection_date")) |>
  filter(
    sensor_type == "temperature",
    real_sensor < sensor_maximum
  ) |>
...
hourly_temp_detections <- valid_temp_detections |>
  mutate(detection_hour = floor_date(detection_datetime, "hour")) |>
  group_by(fish_id, detection_date, detection_hour) |>
  summarize(
    median_temp = median(real_sensor),
    .groups = "drop"
  ) |>
  mutate(median_temp_f = (median_temp * 9 / 5) + 32)

temp_histogram <- hourly_temp_detections |>
  mutate(month = month(detection_date)) |>
  ggplot() +
  geom_histogram(aes(x = median_temp_f)) +
  facet_wrap(~month)
temp_histogram
library(plotly)

ggplotly(temp_histogram)

# Define temp bins, just a starting point here

temp_breaks <- c(-Inf, 60, 63, 66, 69, 72, 75, Inf)
...
hourly_temp_binned <- hourly_temp_detections %>%
  mutate(
    month = floor_date(detection_date, "month"),
    temp_bin = cut(
      median_temp_f,
...
fish_month_temp_bins <- hourly_temp_binned %>%
  group_by(fish_id, month, temp_bin) %>%
  summarise(hours = n(), .groups = "drop") %>%
  group_by(fish_id, month) %>%
  mutate(
    total_hours = sum(hours),
...
# # complete by filling in temp bins with
# # 0 if no observations

fish_month_temp_bins_complete <- fish_month_temp_bins %>%
  group_by(fish_id, month) %>%
  complete(temp_bin, fill = list(hours = 0, prop = 0)) %>%
  ungroup() |>
  mutate(
    month_of_year = month(month, label = T),
    obs_month = month(month),
...
write_feather(fish_month_temp_bins_complete, "shiny_pieces/fish_month_temp_bins")

# # summarize temp by location

# # for each individual, collapse to a single depth
# # per hour on a given valid day

hourly_temp_detections_location <- valid_temp_detections |>
  mutate(detection_hour = floor_date(detection_datetime, "hour")) |>
  group_by(fish_id, location_id, detection_date, detection_hour) |>
  summarize(
    median_temp = median(real_sensor),
...
hourly_temp_binned_location <- hourly_temp_detections_location %>%
  mutate(
    month = floor_date(detection_date, "month"),
    temp_bin = cut(
      median_temp_f,
...
# # proportion of hours by fish within a month
# # that were occupied at a given temp bin

fish_month_temp_bins_location <- hourly_temp_binned_location %>%
  group_by(fish_id, location_id, month, temp_bin) %>%
  summarise(hours = n(), .groups = "drop") %>%
  group_by(fish_id, location_id, month) %>%
  mutate(
    total_hours = sum(hours),
...
fish_month_temp_bins_complete_location <- fish_month_temp_bins_location %>%
  group_by(fish_id, location_id, month) %>%
  complete(temp_bin, fill = list(hours = 0, prop = 0)) %>%
  ungroup() |>
  mutate(
...
write_feather(fish_month_temp_bins_complete_location, "shiny_pieces/fish_month_temp_bins_location")
